...
export function activate(context: ExtensionContext) {
  ...
  const serverModule = context.asAbsolutePath(path.join('out', 'dummyserver', 'server.js'));
  // The debug options for the server
  // --inspect=6009: runs the server in Node's Inspector mode so VS Code can attach to the server for debugging
  const debugOptions = { execArgv: ['--nolazy', '--inspect=0.0.0.0:6009'] };

  // If the extension is launched in debug mode then the debug server options are used
  // Otherwise the run options are used
  const haskellServerOptions: ServerOptions = {
    command:
      '/Users/anka/projekt/not-mine/haskell/lsp/dist-newstyle/build/x86_64-osx/ghc-8.10.2/lsp-1.0.0.1/x/lsp-demo-simple-server/build/lsp-demo-simple-server/lsp-demo-simple-server',
  };
  const jsServerOptions: ServerOptions = {
  ...
  const useHaskell = true;
  const serverOptions = useHaskell ? haskellServerOptions : jsServerOptions;
  ...
  function restartL4Lsp() {
    client.stop().finally(() => client.start());
  }
  ...
  context.subscriptions.push(
    ...
    commands.registerCommand('ide-prototype.restartL4', () => restartL4Lsp()),
  );
}
