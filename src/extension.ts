...
export function activate(context: vscode.ExtensionContext) {
  ...
  const myProvider = new (class implements vscode.TextDocumentContentProvider {
    provideTextDocumentContent(uri: vscode.Uri): string {
      const displayText =
        'The Sale of Cabbages is Restricted. Everybody may sell the item, when the item is a cabbage and sale is onLegalDate or (unlikely) the seller has Exemption.';
      return displayText;
    }
  })();
  ...
  // Generate text panel
  context.subscriptions.push(
    vscode.commands.registerCommand('ide-prototype.mkMarkdown', async () => {
      const currentPanel = vscode.window.activeTextEditor
        ? vscode.window.activeTextEditor.viewColumn
        : undefined;
